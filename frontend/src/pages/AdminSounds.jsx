...
const Admin = () => {
  ...
  const handleForceSyncDeals = async () => {
    if (!confirm('Detta synkar alla deals från Adversus (kan ta 2-5 minuter). Fortsätt?')) return;
    ...
    try {
      ...
      setSyncProgress(`✅ Klart! Synkade ${data.deals} deals från Adversus.`);
      ...
      setSyncProgress(`❌ Fel: ${error.message}`);
      ...
      {showSyncModal && (
        <div className="modal-overlay">
          <div className="modal" style={{ textAlign: 'center', padding: '3rem' }}>
            <h2 style={{ marginBottom: '2rem' }}>🔄 Synkroniserar Deals Cache</h2>
            ...
            {!syncProgress.includes('✅') && !syncProgress.includes('❌') && (
              <div style={{
                width: '60px',
                height: '60px',
            ...
            <p style={{ 
              ...
              {!syncProgress.includes('✅') && !syncProgress.includes('❌') && 
                'Detta kan ta 2-5 minuter beroende på antal deals...'
              }
            </p>
          </div>
        </div>
